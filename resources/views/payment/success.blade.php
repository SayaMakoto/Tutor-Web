@php
    ...
    $theme = $isTutor
        ? [
            'text'     => 'text-green-600',
            'gradFrom' => 'from-green-600', 'gradTo' => 'to-indigo-600',
...
<div class="max-w-md mx-auto text-center py-8 space-y-6">
    ...
    {{-- Success Animation --}}
    <div class="relative">
        <div class="w-24 h-24 bg-gradient-to-br from-emerald-400 to-teal-500 rounded-full
                    flex items-center justify-center mx-auto shadow-xl
                    animate-bounce" style="animation: successPop 0.5s ease-out forwards;">
            <i class="fas fa-check text-white text-4xl"></i>
        </div>
        {{-- Confetti dots --}}
        <div class="absolute top-0 left-1/4 w-3 h-3 bg-yellow-400 rounded-full animate-ping" style="animation-delay:0.2s"></div>
        <div class="absolute top-2 right-1/4 w-2 h-2 bg-blue-400 rounded-full animate-ping" style="animation-delay:0.4s"></div>
        <div class="absolute -top-2 left-1/2 w-2 h-2 bg-pink-400 rounded-full animate-ping" style="animation-delay:0.6s"></div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 text-left space-y-3">
        ...
        <div class="flex justify-between items-center">
            ...
            <span class="text-2xl font-bold {{ $theme['text'] }}">+{{ number_format($order->coin_amount ?? 0) }} Xu</span>
        </div>
        <div class="flex justify-between items-center">
    ...
    <div class="space-y-3">
        <a href="{{ route('payment.wallet') }}"
           class="block w-full bg-gradient-to-r {{ $theme['gradFrom'] }} {{ $theme['gradTo'] }} text-white
