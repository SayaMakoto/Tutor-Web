@php
    ...
    $theme = $isTutor
        ? [
            ...
            'solid'    => 'bg-green-600',   'solidHover' => 'hover:bg-green-700',
            'text'     => 'text-green-600', 'textDark'   => 'text-green-800', 'text700' => 'text-green-700',
        ...
        : [
            'gradFrom' => 'from-blue-600', 'gradTo' => 'to-indigo-700',
            'solid'    => 'bg-blue-600',   'solidHover' => 'hover:bg-blue-700',
            'text'     => 'text-blue-600', 'textDark'   => 'text-blue-800', 'text700' => 'text-blue-700',
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('payment.topup') }}"
           class="inline-flex items-center gap-2 bg-gradient-to-r {{ $theme['btnGradFrom'] }} {{ $theme['btnGradTo'] }}
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {{-- Xu khả dụng --}}
        <div class="bg-gradient-to-br {{ $theme['gradFrom'] }} {{ $theme['gradTo'] }} rounded-2xl p-6 text-white relative overflow-hidden">
    ...
    <div class="{{ $theme['bg50'] }} border {{ $theme['border'] }} rounded-2xl p-5 flex items-start gap-4">
        <div class="w-10 h-10 {{ $theme['bg100'] }} rounded-xl flex items-center justify-center flex-shrink-0">
        ...
        <div>
            ...
            <ul class="{{ $theme['text700'] }} text-xs space-y-1">
                <li><i class="fas fa-check-circle mr-1"></i> <strong>1 Xu = 1.000 VNĐ</strong> — Nạp tiền thật để đổi lấy Xu</li>
        ...
        @if(isset($transactions) && $transactions->count())
            <div class="divide-y divide-gray-50">
                @foreach($transactions as $txn)
                    @php
                        $typeConfig = [
                            'topup'   => ['icon' => 'fas fa-plus-circle',    'color' => 'text-emerald-600', 'bg' => 'bg-emerald-50', 'label' => 'Nạp Xu',       'sign' => '+'],
                            'hold'    => ['icon' => 'fas fa-lock',           'color' => 'text-amber-600',   'bg' => 'bg-amber-50',   'label' => 'Tạm giữ',      'sign' => '-'],
                            'charge'  => ['icon' => 'fas fa-circle-minus',   'color' => 'text-red-500',     'bg' => 'bg-red-50',     'label' => 'Khấu trừ',     'sign' => '-'],
                            'refund'  => ['icon' => 'fas fa-rotate-left',    'color' => 'text-blue-600',    'bg' => 'bg-blue-50',    'label' => 'Hoàn tiền',    'sign' => '+'],
                            'release' => ['icon' => 'fas fa-lock-open',      'color' => 'text-indigo-600',  'bg' => 'bg-indigo-50',  'label' => 'Giải phóng',   'sign' => '+'],
                        ];
                        $cfg = $typeConfig[$txn->type] ?? $typeConfig['topup'];
                    @endphp
                    <div class="px-6 py-4 flex items-center gap-4 hover:bg-gray-50 transition">
                        <div class="w-10 h-10 {{ $cfg['bg'] }} rounded-xl flex items-center justify-center flex-shrink-0">
                        ...
                        <div class="text-right">
                            <p class="font-bold {{ in_array($txn->type, ['topup','refund','release']) ? 'text-emerald-600' : 'text-red-500' }} text-sm">
