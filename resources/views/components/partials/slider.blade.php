...
<div class="relative bg-linear-to-br from-blue-600 via-indigo-600 to-purple-700 rounded-2xl mb-10 overflow-hidden">

    {{-- Decorative background shapes --}}
    <div
        class="absolute top-0 right-0 w-72 h-72 bg-white/5 rounded-full -translate-y-1/3 translate-x-1/4 pointer-events-none">
    </div>
    <div
        class="absolute bottom-0 left-0 w-56 h-56 bg-white/5 rounded-full translate-y-1/3 -translate-x-1/4 pointer-events-none">
    </div>
    ...
    <div class="relative z-10 px-8 md:px-16 py-14 md:py-20 text-white text-center">
        ...
        {{-- Badge --}}
        <div
            class="inline-flex items-center gap-2 bg-white/15 border border-white/20 
        ...
        @php
            ...
            $searchLabel = 'Tìm gia sư ngay';

            // Nếu người dùng đã đăng nhập và có vai trò là gia sư (tutor)
            if (auth()->check() && auth()->user()->role === 'tutor') {
                $searchRoute = route('tutor.classes.index');
                $searchLabel = 'Tìm lớp đang tuyển ngay';
            }
        @endphp
